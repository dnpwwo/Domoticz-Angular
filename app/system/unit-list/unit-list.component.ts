...
import { Unit, UnitsMessage } from "../unit";
import { UnitComponent } from '../unit/unit.component';
import { ConfirmationDialogComponent } from 'src/app/system/confirmation-dialog/confirmation-dialog.component';
import { UnitListWatcherService } from 'src/app/system/unit-list/unit-list-watcher.service';
...
export class UnitListComponent implements AfterViewInit, OnDestroy {

  tableColumns: string[] = ['UnitID', 'Name', 'Minimum', 'Maximum', 'IconList', 'TextLabels'];
  selectedRow: Unit;
  selectedRowNumber = 0;
  ...
  loadData() {
    this.unitService.getUnits().subscribe((UnitsMsg: UnitsMessage) => {
      if (UnitsMsg != null) {
        this.dataSource = new MatTableDataSource(UnitsMsg.Units);
        this.snackBar.open(UnitsMsg.Count + ' Unit' + (this.dataSource.data.length > 1 ? 's' : '') + ' loaded. ', '', { duration: 3000 });
        this.dataSource.paginator = this.paginator;
        this.dataSource.sort = this.sort;
        this.dataSource.filterPredicate =
          (data: Unit, filtersJson: string) => {
            const matchFilter = [];
      ...
      else this.snackBar.open('No Units returned. ', '', { duration: 5000 });
      return;
    }, (error) => {
  ...
  subscribe() {
    return new UnitListWatcherService(
      () => console.log("Units Websocket connected."),
      () => this.getItems(),
      () => {
        console.log("Units Websocket disconnected."),
          this.unsubscribe();
      })
      .subscribe(
        (values: Unit[], reason: string) => {
  ...
  unsubscribe() {
    if (typeof (this.webSocketSubscriber) != "undefined") {
      ...
      if (this.isActive) {
        this.snackBar.open("Unit List socket has disconnected.", 'Retry').onAction().subscribe(() => { this.ngAfterViewInit(); });
      }
    }
  }
  ...
  onNew() {
    this.dialogConfig.data = new Unit();
    const dialogRef = this.dialog.open(UnitComponent, this.dialogConfig);
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.unitService.createUnit(result);
      }
    });
  }
  ...
  onEdit() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.unitService.updateUnit(this.selectedRow.UnitID, result);
      }
    });
  }
