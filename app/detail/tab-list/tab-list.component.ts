...
export class TabListComponent implements AfterViewInit, OnDestroy {

  //tableColumns: string[] = ['TabID', 'LayoutID', 'Name', 'DisplayOrder', 'Icon', 'RowHeight', 'MinColumns', 'MaxColumns', 'MinColWidth', 'MaxColWidth', 'GutterSize'];
  tableColumns: string[] = ['TabID', 'LayoutID', 'Name', 'DisplayOrder', 'Icon'];
  selectedRow: Tab;
  selectedRowNumber = 0;
  ...
  loadData() {
    this.tabService.getTabs(this.route.snapshot.params['id']).subscribe((TabsMsg: any) => {
      debugger;
      if (TabsMsg != null) {
        this.dataSource = new MatTableDataSource(TabsMsg.Tabs);
        this.snackBar.open(TabsMsg.Count + ' Tab' + (this.dataSource.data.length > 1 ? 's' : '') + ' loaded. ', '', { duration: 3000 });
        this.dataSource.paginator = this.paginator;
        this.dataSource.sort = this.sort;
        this.dataSource.filterPredicate =
          (data: Tab, filtersJson: string) => {
            const matchFilter = [];
  ...
  onNew() {
    this.dialogConfig.data = new Tab();
    this.dialogConfig.data.LayoutID = this.route.snapshot.params['id'];
    const dialogRef = this.dialog.open(TabComponent, this.dialogConfig);
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.tabService.createTab(result);
      }
    });
  }
  ...
  onEdit() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.tabService.updateTab(this.selectedRow.TabID, result);
      }
    });
  }
  ...
  onDelete() {
    const dialogRef = this.deleteDialog.open(ConfirmationDialogComponent, {
      width: '75%',
      data: "Please confirm deletion of tab '" + this.selectedRow.Name + "'?"
    });
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.tabService.deleteTab(this.selectedRow.TabID);
